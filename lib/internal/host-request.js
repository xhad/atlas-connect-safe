...
HostClient.prototype.asUser = function (userKey) {
  ...
  console.warn('This has been deprecated, as per https://ecosystem.atlassian.net/browse/ACEJS-115')
...
HostClient.prototype.asUserByAccountId = function (userAccountId) {
  if (!userAccountId) {
    throw new Error('A userAccountId must be provided to make a request as a user')
  }

  var product = this.addon.config.product()
  if (!product.isJIRA && !product.isConfluence) {
    throw new Error('the asUserByAccountId method is not available for ' + product.id + ' add-ons')
  }
...
HostClient.prototype.createJwtPayload = function (req) {
  ...
  var jwtTokenValidityInMinutes = this.addon.config.jwt().validityInMinutes
...
HostClient.prototype.getUserBearerToken = function (scopes, clientSettings) {
  ...
  if (this.userAccountId) {
    return this.oauth2.getUserBearerTokenByUserAccountId(this.userAccountId, scopes, clientSettings)
  } else if (this.userKey) {
    return this.oauth2.getUserBearerToken(this.userKey, scopes, clientSettings)
  } else {
    throw new Error('One of userAccountId or userKey must be provided. Did you call asUserByAccountId(userAccountId)?')
  }
};
      ...
      .then(function (clientContext) {
        var augmentHeaders = function (headers, relativeUri) {
          ...
          if (!(self.userKey || self.userAccountId)) {
            ...
            var jwtToken = jwt.encode(jwtPayload, clientContext.clientSettings.sharedSecret, 'HS256')
