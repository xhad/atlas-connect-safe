...
function createTunnel (addon) {
  var hasRemoteHosts = _.some(addon.config.hosts(), function (host) {
  ...
  return requireNgrok()
    .then(function (ngrok) {
      //return 
      const { ADDON_DOMAIN_HOST } = process.env
      return ADDON_DOMAIN_HOST
        ? ADDON_DOMAIN_HOST 
        : RSVP.denodeify(ngrok.connect)(addon.config.port())
    })
    .then(function (url) {
      var ltu = new URI(url)
      ...
      addon.logger.info('THE ADDON DOMAIN IS', url)
    ...
    .catch(function (err) {
      ...
      if (err.code === 'MODULE_NOT_FOUND') {
        addon.logger.error('Make sure that ngrok is installed: npm install --save-dev ngrok@2')
      }
